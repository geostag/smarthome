...
srcdir = "/var/www/pi/schatzkiste/stats"
        ...
        with open(join(srcdir,fn),"r") as f:
            for line in f.readlines():
                ...
                if m:
                    used = m.group(1)
                    percent = m.group(3)
                    data.append( { "date": d, "used": used, "percent": percent } )

for r in sorted(data,key = lambda x: x["date"]):
    dt = datetime.strptime(r["date"],"%Y-%m-%d")
    dt = dt.replace(tzinfo=ZoneInfo("Europe/Berlin"))
    p = (Point("schatzkiste").tag("domain","storage").field("used",int(r["used"])).time(dt,WritePrecision.NS))
    print(p)
